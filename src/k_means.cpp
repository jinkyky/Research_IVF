...
void KMeans :: fit(const DataSet &data)
{
    ...
    for(int it = 0; it < max_iter; it++) // iterator
    {
        ...
        for(size_t i = 0; i < data.size(); i++)
        {
            ...
            for(int j = 0; j < k; j++)
            {
                double dist = distance<ElemType, float>(data[i], centroids[j]);
                if(dist < min_dist)
                {
                    min_dist = dist;
                    min_index = j;
...
void KMeans::init_centroids(const DataSet &data) //randomly choose k centroids
{
    ...
    for(int i = 0; i < k; )
    {
        int index = dis(gen);
        if(!selected[index]) {
            centroids.push_back(convertVector(data[index]));
            selected[index] = true;
            ++i;
        }
    }
...
{
    VecSet<float> new_centroids(k,std::vector<float>(data[0].size(),0));
    ...
    for(size_t i = 0; i < data.size(); i++)
    {
        ...
        {
            new_centroids[cluster][j] += static_cast<float>(data[i][j]);
        }
        count[cluster]++;
    }
    ...
    for(int c = 0; c < k; c++)
    {
        if(count[c] == 0) 
        {
            ...
            new_centroids[c] = convertVector(data[dis(gen)]);
            continue; // handle empty cluster
        }
        double scale = 1.0 / static_cast<double>(count[c]);
